...
| `isCanvas` | `isCanvas`, `canvasName`, `isSlot`, `slotPropertyName` | Truthy |

**Canvas metadata**: When a node has `isCanvas: true`, the exporter sets `isCanvas`, `canvasName`, and also marks the node as a slot (`isSlot: true`, `slotPropertyName: canvasName`) for Figma plugin compatibility. This dual-flagging allows canvas frames to behave as Figma slots while retaining their canvas rendering semantics.
