...
import { frame, text, rectangle, ellipse, group, component, componentSet, instance, image, line, section, polygon, star, union, subtract, intersect, exclude, slot } from '@figma-dsl/core';
...
// --- Task 2.1: Slot placement validation ---

describe('compile() — slot placement validation', () => {
  it('allows slot inside a COMPONENT', () => {
    const node = component('Card', {
      children: [slot('Content')],
    });
    const result = compile(node);
    expect(result.errors).toEqual([]);
    const slotChild = result.root.children[0]!;
    expect(slotChild.type).toBe('FRAME');
    expect(slotChild.isSlot).toBe(true);
    expect(slotChild.slotName).toBe('Content');
  });

  it('reports error when slot is inside a plain FRAME', () => {
    const node = frame('Page', {
      children: [slot('BadSlot')],
    });
    const result = compile(node);
    expect(result.errors.length).toBeGreaterThan(0);
  ...
  it('allows slot inside a COMPONENT_SET child component', () => {
    const node = componentSet('Buttons', {
      children: [
    ...
    const result = compile(node);
    // No errors about slot placement
    const slotErrors = result.errors.filter(e => e.message.includes('slot'));
    expect(slotErrors).toEqual([]);
  });

  it('allows nested slot within a child frame of a component', () => {
    const node = component('Card', {
      children: [
        frame('Inner', {
    ...
    const slotErrors = result.errors.filter(e => e.message.includes('slot'));
    expect(slotErrors).toEqual([]);
    // Slot should be at children[0].children[0]
    const slotNode = result.root.children[0]!.children[0]!;
    expect(slotNode.isSlot).toBe(true);
    expect(slotNode.slotName).toBe('Actions');
  });

  it('passes through isSlot and slotName to compiled output', () => {
    const node = component('Card', {
      children: [
        slot('Header', { size: { x: 200, y: 50 } }),
        slot('Body'),
      ],
    });
    const result = compile(node);
    expect(result.root.children[0]!.isSlot).toBe(true);
    expect(result.root.children[0]!.slotName).toBe('Header');
    expect(result.root.children[1]!.isSlot).toBe(true);
    expect(result.root.children[1]!.slotName).toBe('Body');
  });
});

// --- Task 2.2: SLOT property injection ---

describe('compile() — SLOT property injection', () => {
  it('injects SLOT property definition for each slot child', () => {
    const node = component('Card', {
      children: [
        slot('Header'),
        text('Title'),
    ...
    const result = compile(node);
    expect(result.root.componentPropertyDefinitions).toBeDefined();
    expect(result.root.componentPropertyDefinitions!['Header']).toEqual({
      type: 'SLOT',
      defaultValue: '',
    });
    expect(result.root.componentPropertyDefinitions!['Footer']).toEqual({
      type: 'SLOT',
      defaultValue: '',
    });
  });

  it('does not inject SLOT properties when component has no slots', () => {
    const node = component('Button', {
      children: [text('Click')],
    });
    const result = compile(node);
    // No componentPropertyDefinitions at all (no explicit props defined)
    expect(result.root.componentPropertyDefinitions).toBeUndefined();
  });

  it('merges SLOT properties with explicit component properties', () => {
    const node = component('Card', {
      componentProperties: [
        { name: 'Title', type: 'TEXT', defaultValue: 'Hello' },
      ],
    ...
    const result = compile(node);
    expect(result.root.componentPropertyDefinitions!['Title']).toEqual({
      type: 'TEXT',
      defaultValue: 'Hello',
    });
    expect(result.root.componentPropertyDefinitions!['Content']).toEqual({
      type: 'SLOT',
      defaultValue: '',
    });
  });

  it('accepts SLOT as a valid component property type', () => {
    const node = component('Widget', {
      componentProperties: [
        { name: 'Content', type: 'SLOT', defaultValue: '' },
      ],
    });
    const result = compile(node);
    // Should not produce an error for SLOT type
    const slotTypeErrors = result.errors.filter(e => e.message.includes("'SLOT'"));
    expect(slotTypeErrors).toEqual([]);
    expect(result.root.componentPropertyDefinitions!['Content']).toEqual({
      type: 'SLOT',
      defaultValue: '',
    });
...
// --- Task 2.3: Slot overrides on instance nodes ---

describe('compile() — instance slot overrides', () => {
  it('compiles slot overrides on instance nodes', () => {
    const node = frame('Page', {
      children: [
    ...
    expect(inst.slotOverrides!['Content']![0]!.type).toBe('TEXT');
    expect(inst.slotOverrides!['Content']![0]!.textData?.characters).toBe('Custom text');
  });

  it('compiles multiple slot overrides', () => {
    const node = frame('Page', {
      children: [
        instance('Layout', undefined, {
          Header: [text('Title')],
          Footer: [text('Copyright'), text('2024')],
        }),
      ],
    });
    const result = compile(node);
  ...
  it('does not set slotOverrides when instance has none', () => {
    const node = frame('Page', {
      children: [instance('Button', { Label: 'Click' })],
    });
    const result = compile(node);
    const inst = result.root.children[0]!;
    expect(inst.slotOverrides).toBeUndefined();
  });

  it('recursively compiles override content nodes', () => {
    const node = frame('Page', {
      children: [
        instance('Card', undefined, {
          Content: [
    ...
    const result = compile(node);
    const inst = result.root.children[0]!;
    const overrideFrame = inst.slotOverrides!['Content']![0]!;
    expect(overrideFrame.type).toBe('FRAME');
    expect(overrideFrame.stackMode).toBe('VERTICAL');
    expect(overrideFrame.itemSpacing).toBe(8);
    expect(overrideFrame.children).toHaveLength(2);
  });
});
