  ...
  return (
    <div data-testid="root" style={{ padding: 24, backgroundColor: '#fff', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Badge label="Node.js" color="#68a063" />
          <Badge label="CSS" color="#264de4" />
        </div>
        <SectionHeader title="Tags" />
        <div style={{ display: 'flex', flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <Tag label="React" color="#61dafb" />
          <Tag label="TypeScript" color="#3178c6" />
          <Tag label="Node.js" color="#68a063" />
          <Tag label="CSS" color="#264de4" />
        </div>
        <SectionHeader title="Chips" />
        <div style={{ display: 'flex', flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          <Chip label="React" selected={true} color="#61dafb" />
          <Chip label="TypeScript" selected={false} color="#3178c6" />
          <Chip label="Node.js" selected={false} color="#68a063" />
          <Chip label="CSS" selected={false} color="#264de4" />
        </div>
    </div>
  );
}
