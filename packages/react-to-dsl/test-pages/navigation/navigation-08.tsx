...
export default function navigation_08() {
  return (
    <div data-testid="root" style={{ backgroundColor: '#f5f5f5' }}>
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#ffffff', padding: '12px 24px' }}>
          <span style={{ fontSize: 18, fontWeight: 700, color: '#333333' }}>Logo</span>
          <div style={{ display: 'flex', flexDirection: 'row', gap: 8, alignItems: 'center' }}>
            <NavItem label="Home" active={true} color="#333333" />
            <NavItem label="About" active={false} color="#333333" />
            <NavItem label="Products" active={false} color="#333333" />
            <NavItem label="Contact" active={false} color="#333333" />
            <NavItem label="Blog" active={false} color="#333333" />
          </div>
          <Avatar color="#333" size={32} />
        </div>
    </div>
  );
}
