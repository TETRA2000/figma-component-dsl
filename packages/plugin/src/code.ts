...
function serializeFills(node: SceneNode): PluginNodeDef['fills'] {
  ...
  return fills.filter((f): f is SolidPaint | GradientPaint | ImagePaint => f.visible !== false).map(f => {
    if (f.type === 'SOLID') {
      return {
        type: 'SOLID',
    ...
    if (f.type === 'IMAGE') {
      const imgPaint = f as ImagePaint;
      return {
        type: 'IMAGE',
        opacity: imgPaint.opacity ?? 1,
        imageHash: imgPaint.imageHash,
        imageScaleMode: imgPaint.scaleMode,
      };
    }
