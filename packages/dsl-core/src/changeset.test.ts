...
describe('Changeset Types', () => {
  ...
  // --- Task 8.2: Slot-specific changeset fields ---

  it('PropertyChange supports slotName field', () => {
    const change: PropertyChange = {
      propertyPath: 'children.0',
      changeType: 'added',
      description: 'Child added to slot',
      slotName: 'Content',
      slotChangeType: 'child-added',
      slotContent: [{ type: 'TEXT', name: 'New text' }],
    };
    expect(change.slotName).toBe('Content');
    expect(change.slotChangeType).toBe('child-added');
    expect(change.slotContent).toBeDefined();
  });

  it('PropertyChange slot fields are optional (backward compatible)', () => {
    const change: PropertyChange = {
      propertyPath: 'fills.0.color.r',
      changeType: 'modified',
      description: 'Color change',
    };
    expect(change.slotName).toBeUndefined();
    expect(change.slotChangeType).toBeUndefined();
    expect(change.slotContent).toBeUndefined();
  });

  it('slotChangeType supports all slot change types', () => {
    const types: Array<PropertyChange['slotChangeType']> = ['child-added', 'child-removed', 'child-reordered'];
    expect(types).toHaveLength(3);
  });

  it('changeset with slot changes contains serialized children', () => {
    const doc: ChangesetDocument = {
      schemaVersion: '1.0',
      ...
      components: [{
        ...
        componentId: 'node-1',
        changes: [{
          propertyPath: 'children.1',
          changeType: 'added',
          description: 'Child added to slot Content',
          slotName: 'Content',
          slotChangeType: 'child-added',
          slotContent: [
            { type: 'TEXT', name: 'New text', size: { x: 100, y: 20 }, opacity: 1, visible: true, children: [] },
          ],
        }],
      }],
    };
    expect(doc.components[0].changes[0].slotName).toBe('Content');
    expect(doc.components[0].changes[0].slotContent).toHaveLength(1);
  });
});
